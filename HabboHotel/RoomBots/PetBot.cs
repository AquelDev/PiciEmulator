...
namespace Pici.HabboHotel.RoomBots
{
    class PetBot : BotAI
    {
        ...
        internal override void OnUserSay(Rooms.RoomUser User, string Message)
        {
            ...
            if (Message.ToLower().StartsWith(Pet.PetData.Name.ToLower() + " ") && User.GetClient().GetHabbo().Username.ToLower() == GetRoomUser().PetData.OwnerName.ToLower())
            {
                ...
                int r = PiciEnvironment.GetRandomNumber(1, 8); // Made Random
        ...
        internal override void OnTimerTick()
        {
            ...
            if (SpeechTimer <= 0)
            {
                ...
                SpeechTimer = PiciEnvironment.GetRandomNumber(20, 120);
            }
            else
            {
                SpeechTimer--;
            ...
            if (ActionTimer <= 0)
            {
                try
                {
                    ...
                    ActionTimer = PiciEnvironment.GetRandomNumber(15, 40 + GetRoomUser().PetData.VirtualId);
                }
                catch (Exception e)
                {
            ...
            if (EnergyTimer <= 0)
            {
                ...
                EnergyTimer = PiciEnvironment.GetRandomNumber(30, 120); // 2 Min Max
            }
            else
            {
                EnergyTimer--;
