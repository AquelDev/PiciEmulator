...
using Pici.Core;
using Pici.HabboHotel.Items.Interactors;
            ...
            if (this.UpdateCounter <= 0 || IsTrans)
            {
                ...
                switch (GetBaseItem().InteractionType)
                {
                                        ...
                                        if (true)
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                if (!User.IsBot && User != null && User.GetClient() != null && User.GetClient().GetHabbo() != null && User.GetClient().GetMessageHandler() != null)
                                                {
                                                    ...
                                                    //PiciEnvironment.GetGame().GetRoomManager().AddTeleAction(new TeleUserData(User.GetClient().GetMessageHandler(), User.GetClient().GetHabbo(), RoomId, TeleId));
                                            }
                    ...
                    case InteractionType.bottle:
                        ...
                        ExtraData = PiciEnvironment.GetRandomNumber(0, 7).ToString();
                        UpdateState();
                        break;

                    case InteractionType.dice:

                        ExtraData = PiciEnvironment.GetRandomNumber(1, 6).ToString();
                        UpdateState();
                        break;

                    case InteractionType.habbowheel:

                        ExtraData = PiciEnvironment.GetRandomNumber(1, 10).ToString();
                        UpdateState();
                        break;
                    ...
                    case InteractionType.loveshuffler:
                        ...
                        if (ExtraData == "0")
                        {
                            ExtraData = PiciEnvironment.GetRandomNumber(1, 4).ToString();
                            ReqUpdate(20, false);
                        }
                        else if (ExtraData != "-1")
                        {
                    ...
                    case InteractionType.vendingmachine:
                        ...
                        if (this.ExtraData == "1")
                        {
                            ...
                            if (User != null)
                            {
                                ...
                                int randomDrink = GetBaseItem().VendingIds[PiciEnvironment.GetRandomNumber(0, (GetBaseItem().VendingIds.Count - 1))];
